/**
 * License: https://github.com/fex-team/fis-spriter-csssprites/blob/master/libs/image.js
 * make some changes
 */
...
function Generator(file, index, list, images, ret, settings, opt) {
  ...
  her.util.map(default_settings, function (key, value) {
  ...
  her.util.map(list, function (k, bg) {
    var image_ = Image(getImage(bg.getImageUrl()).contents);
  ...
  //background-size
  her.util.map(scales, function (s, l) {
    s = parseFloat(s);
    that.zFill(l['z'], s);
  });
}
...
Generator.prototype = {
  ...
  after: function (image, arr_selector, direct, scale) {
    ...
    var fileName = this.file.pathNoExt + ext;
    var vinyl = new File({
      cwd: this.file.cwd,
      base: this.file.base,
      path: fileName //temp path,not real
    });
    vinyl.release = this.file.release; //root path
    var image_file = her.file(vinyl);
    image_file.contents = image.encode('png');
    //fis.compile(image_file);
    this.ret.pkg[image_file.id] = image_file;
